...
// .wrangler/tmp/bundle-fdOr95/checked-fetch.js
...
// .wrangler/tmp/bundle-fdOr95/strip-cf-connecting-ip-header.js
...
function createErrorResponse(error, requestId) {
  ...
  if (error instanceof ApiError) {
    ...
    if (error.code) {
      errorResponse2.code = error.code;
    }
    if (error.details) {
      errorResponse2.details = error.details;
    }
...
function createSuccessResponse(data, status = 200, requestId) {
  const id = requestId || generateRequestId();
  const response = {
    success: true,
    data,
...
// src/types.ts
var ExternalApiProxyRequestSchema = external_exports.object({
  target_service: external_exports.enum([
    "openai_chat",
    "openai_embedding",
    "anthropic_claude",
    "jina_embedding",
  ...
  api_key: external_exports.string().min(1, "API key is required"),
  payload: external_exports.record(external_exports.unknown())
  // Service-specific payload as generic object
});
var VectorQuerySchema = external_exports.object({
  query: external_exports.string().min(1),
  projectId: external_exports.string().uuid(),
  topK: external_exports.number().int().min(1).max(50).optional().default(10),
  filters: external_exports.record(external_exports.unknown()).optional()
});

// src/lib/external-service-configs.ts
var EXTERNAL_SERVICE_CONFIGS = {
  // OpenAI Services
  openai_chat: {
...
__name(getServiceConfig, "getServiceConfig");
function buildExternalHeaders(service, apiKey, additionalHeaders) {
...
__name(buildExternalHeaders, "buildExternalHeaders");
function validateExternalService(service) {
  return service in EXTERNAL_SERVICE_CONFIGS;
}
__name(validateExternalService, "validateExternalService");

// src/handlers/proxy-handlers.ts
async function proxyExternalApiHandler(c) {
...
__name(proxyExternalApiHandler, "proxyExternalApiHandler");
async function proxyHealthHandler(c) {
  const requestId = c.get("requestId");
...
__name(proxyHealthHandler, "proxyHealthHandler");
async function proxySupportedServicesHandler(c) {
  const requestId = c.get("requestId");
...
__name(proxySupportedServicesHandler, "proxySupportedServicesHandler");

// src/handlers/debug-handlers.ts
var EchoRequestSchema = external_exports.unknown();
async function echoHandler(c) {
  try {
    const body = await c.req.json();
    return c.json({
      success: true,
...
__name(echoHandler, "echoHandler");
...
// .wrangler/tmp/bundle-fdOr95/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fdOr95/middleware-loader.entry.ts
